...
export function useSync(currentUser: string, onDataUpdate: (data: SyncData) => void) {
  ...
  // Funzione per salvare i dati sincronizzati
  const saveSyncData = useCallback((data: SyncData) => {
    if (isUpdating.current) return

    const syncKey = "app_sync_data"
    const syncTimestampKey = "app_sync_timestamp"

    localStorage.setItem(syncKey, JSON.stringify(data))
    localStorage.setItem(syncTimestampKey, data.timestamp.toString())
      ...
      try {
        ...
        if (data.lastUpdatedBy !== currentUser) {
          toast({
            ...
            description: `Sincronizzazione automatica da ${data.lastUpdatedBy}`,
            duration: 3000,
          })
        }
      } finally {
  ...
  const checkForUpdates = useCallback(() => {
    ...
    const savedTimestamp = localStorage.getItem(syncTimestampKey)
  ...
  useEffect(() => {
    ...
    // Imposta il polling per controllare gli aggiornamenti ogni 2 secondi
    syncInterval.current = setInterval(checkForUpdates, 2000)

    // Listener per gli eventi di storage (per sincronizzazione cross-tab)
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === "app_sync_timestamp" && e.newValue) {
        checkForUpdates()
      }
    }
